  ...
  const renderRideList = () => {
    if (rideRequests && rideRequests.length !== 0) {
      return rideRequests.map(request => (
        <div className="ride-list__result-item" key={(request)?request.rideUuid:0}>
          ...
          <div>
            {request&&request.ride && <p className="ride-list__result-label"><span>From: </span>{request.ride.pickup && request.ride.pickup.label ? request.ride.pickup.label : ''}</p>}
            {request&&request.ride && <p className="ride-list__result-label"><span>To: </span>{request.ride.destination && request.ride.destination.label ? request.ride.destination.label : ''}</p>}
